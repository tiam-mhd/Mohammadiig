...
پیشنهاد:
1. یک Persistent Volume (مثلاً `/data`) به اپ بک‌اند وصل کنید — **بدون Volume پایدار، با هر Redeploy عکس‌ها پاک می‌شوند و کتابخانه رسانه خالی/شکسته دیده می‌شود.**
2. `MEDIA_ROOT=/data/media` بگذارید (روی همان Volume).
3. `MEDIA_PUBLIC_BASE_URL` را خالی بگذارید تا در دیتابیس فقط مسیر نسبی `/media/...` ذخیره شود.
