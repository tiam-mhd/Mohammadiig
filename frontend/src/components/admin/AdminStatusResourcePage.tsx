...
export function AdminStatusResourcePage<T extends { id: string }>({
  ...
  useEffect(() => {
    if (!token) return;
    fetchItems(token).then(setItems).catch(() => adminToast.error(`دریافت ${resourceLabel} انجام نشد.`));
  ...
  async function saveEdit(event: FormEvent) {
    ...
    try {
      ...
      setEditing(null);
      adminToast.success(`${resourceLabel} به‌روزرسانی شد.`);
    } catch (error) {
  ...
  async function confirmRemove() {
    ...
    try {
      ...
      adminToast.success(`${resourceLabel} لغو شد.`);
    } catch (error) {
