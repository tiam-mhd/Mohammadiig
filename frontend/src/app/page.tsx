...
const sampleProducts = [
  { id: '1', name: 'Bumper Car / سری Signature', description: 'بدنه مهندسی شده، ایمنی بالا و آماده برای بهره برداری حرفه ای.', price: 45000000, category: 'EQUIPMENT', image: 'https://images.unsplash.com/photo-1565610222536-ef125c59da2e?auto=format&fit=crop&w=900&q=85' },
  { id: '2', name: 'Junior / سری Play', description: 'نسخه ایمن و پرانرژی برای تجربه ای ماندگار در فضاهای خانوادگی.', price: 15000000, category: 'FAMILY', image: 'https://images.unsplash.com/photo-1531058020387-3be344556be6?auto=format&fit=crop&w=900&q=85' },
  { id: '3', name: 'MIG Care / قطعات یدکی', description: 'قطعات اصلی و پشتیبانی فنی برای حفظ عملکرد بلندمدت مجموعه.', price: 8500000, category: 'AFTER-SALES', image: 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=900&q=85' },
];
...
export default function Home() {
  ...
  <section className="relative border-t border-white/10 bg-neutral-900 py-24 text-white sm:py-32"><div className="absolute left-0 top-0 h-full w-1/3 bg-[radial-gradient(circle_at_20%_30%,rgba(185,137,82,.18),transparent_55%)]" /><div className="relative mx-auto flex max-w-[1400px] flex-col justify-between gap-12 px-5 sm:px-8 lg:flex-row lg:items-end"><div><span className="eyebrow">02 / THE MIG METHOD</span><h2 className="mt-5 max-w-2xl text-4xl font-black leading-tight sm:text-6xl">هر پروژه،<br /><span className="text-primary-500">یک امضای تازه.</span></h2></div><div className="max-w-md"><p className="text-base leading-8 text-neutral-300">ما فقط محصول نمی سازیم. از تولید تجهیزات تا بهره برداری مجموعه های تفریحی و سرمایه گذاری در ایده های آینده، کنار شما می ایستیم.</p><Link href="/about" className="mt-8 inline-block border-b border-primary-500 pb-2 text-sm font-bold text-primary-500">بیشتر درباره MIG <span className="mr-2">←</span></Link></div></div></section>
      <section className="bg-primary-500 px-5 py-16 text-neutral-900 sm:px-8 sm:py-20"><div className="mx-auto flex max-w-[1400px] flex-col justify-between gap-8 sm:flex-row sm:items-center"><div><span className="text-xs font-black uppercase tracking-[0.18em]">LET'S BUILD WHAT'S NEXT</span><h2 className="mt-3 text-3xl font-black sm:text-5xl">پروژه بعدی شما از اینجا شروع می شود.</h2></div><Link href="/contact"><Button size="lg" className="w-fit rounded-none bg-neutral-900 px-8 text-white hover:bg-neutral-800">گفتگو با تیم ما <span>←</span></Button></Link></div></section>
    </main>
  );
}
